...
options = trainingOptions('sgdm','MaxEpochs',MaxEpochs, ...
    ...
    'CheckpointPath', 'CNN\model_checkPoint');
...
not_long=0;
not_short=0;
for num = 1:numel(valPredict)
    if valPredict(num) == valLabel(num)
        switch double(valPredict(num))
            case {1,3}
                num_long= num_long+1;
            case {2,4}
                num_short = num_short+1;
        end
    elseif valPredict(num) ~= valLabel(num)
        switch double(valPredict(num))
            case {1,3}
                not_long = not_long+1;
            case {2,4}
                not_short= not_short+1;
        end
    end
end
approximate_accuracy = (num_long+num_short)/numel(valLabel);

confusion_matrix ={'' , 'Predict L', 'Predict S'; 'Actual L', num_long, not_short; 'Actual S' not_long, num_short}
...
%%
load CNN\model_checkPoint\convnet_checkpoint__21__2017_08_30__15_58_55.mat % =net 

valPredict2 = classify(net,valData);
sum(valPredict == valPredict2);
